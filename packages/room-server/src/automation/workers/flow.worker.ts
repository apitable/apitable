...
import { RobotTriggerService, AutomationService } from '../services';
...
export class FlowWorker {
  ...
  public start(): void {
    this.flowWorker = new Worker(
      FLOW_QUEUE,
      async (job) => await this.onProcess(job),
      {
  ...
  private async triggerFlows(datasheetTriggers: ResourceRobotTriggerDto[], triggerFactory: ITriggerFactory<any> | undefined, extra: any): Promise<void> {
    if (!triggerFactory) {
