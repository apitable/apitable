...
export class MemberField extends BaseField implements OnApplicationBootstrap {
  ...
  override validate(fieldValue: IFieldValue, field: IField, extra?: { [key: string]: string }) {
    ...
    for (const key of keys) {
      ...
      if (!fieldValue[key].name) this.throwException(field, ApiTipConstant.api_params_instance_member_name_error);
      if (!fieldValue[key].type) this.throwException(field, ApiTipConstant.api_params_instance_member_type_error);
    }
  }
