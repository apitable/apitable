...
Each option can set its own pre-icon and suffix-icon, the Icon is imported from the `@vikadata/icons` library.
...
<ThemeToggle lang="en">
    <Select
        options={[
            { label: 'test label 1', value: 'opt1', prefixIcon: <FavoriteOutlined />, suffixIcon: <FavoriteOutlined /> },
            { label: 'test label 2', value: 'opt2', prefixIcon: <FavoriteOutlined />, suffixIcon: <FavoriteOutlined /> },
        ]}
        value="opt1"
        dropdownMatchSelectWidth={false}
        triggerStyle={{ width: 100 }}
