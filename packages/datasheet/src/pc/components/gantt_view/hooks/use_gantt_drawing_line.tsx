...
const Circle = dynamic(
  () => import("pc/components/gantt_view/hooks/use_gantt_timeline/circle"),
  { ssr: false }
);
...
export const useGanttDrawingLine = (props: IDrawingLineProps) => {
  ...
  const sourceRecordId = isTaskLineDrawing
    ? taskBlock.current.id
    : transformerId.split("-")[1];
  ...
  const checkErrorLine = (sourceId: string, targetId: string) => {
    ...
    const targetStartValue = Selectors.getCellValue(
      state,
      snapshot,
      targetId,
      startFieldId
    );
    const sourceEndValue = Selectors.getCellValue(
      state,
      snapshot,
      sourceId,
      endFieldId
    ...
    if (
      targetStartValue &&
      sourceEndValue &&
      targetStartValue <= sourceEndValue
    ) {
      return true;
    }
    ...
    sourceAdjCopy[sourceId] = sourceAdjCopy[sourceId]
      ? sourceAdjCopy[sourceId].push(targetId)
      : [targetId];
  ...
  const onDragMove = (e) => {
    ...
    const noScrollCb = () =>
      setDrawingLinePoints([x, y + 6, x, curY, curX, curY]);
    ...
    if (
      targetRecordId !== "" &&
      checkErrorLine(sourceRecordId, targetRecordId)
    ) {
