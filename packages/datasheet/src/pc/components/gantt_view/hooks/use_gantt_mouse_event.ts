...
import {
  CellType, CollaCommandName, databus, DropDirectionType, FieldType, ICellValue, IGanttViewProperty, IGridViewProperty, ISetRecordOptions, KONVA_DATASHEET_ID,
  Selectors,
} from '@apitable/core';
...
export const useGanttMouseEvent = ({
  ...
  const clickBlankHandler = (): Promise<databus.ICommandExecutionResult<string[]>> | void => {
    const task = getTaskData(pointRowIndex);
    if (task == null) return;
