...
import { INodeTree, IShareSpaceInfo } from '../share';
...
const NodeTree = (
  ...
  const renderNode = (node: INodeTree[] | undefined) => {
    ...
    return node!.map(item => {
      const icon = getNodeIcon(item.icon, item.type, {
        size: 16, emojiSize: 18, actived: item.nodeId === activedNodeId, normalColor: colors.staticWhite0
      });
  ...
  const icon = getNodeIcon(nodeTree.icon, nodeTree.type, {
    size: 16, emojiSize: 18,
    actived: nodeTree.nodeId === activedNodeId, normalColor: colors.staticWhite0,
  });
  ...
  return (
    <DirectoryTree
      defaultExpandAll
      onSelect={onSelect}
      switcherIcon={<span><PullDownIcon fill={colors.staticWhite0} /></span>}
    ...
    <div className={styles.shareMenu}>
      ...
      <div className={styles.shareInfo}>
        <div className={styles.avatar}>
          <Avatar
            src={shareSpace.lastModifiedAvatar}
            title={shareSpace.lastModifiedBy}
            size={AvatarSize.Size80}
            id={shareSpace.spaceId}
          />
        ...
        <>
          {
            ...
            <OperationCard
              img={SavePng}
              tipText={t(Strings.save_action_desc)}
              btnText={t(Strings.save_to_space)}
              onClick={saveToMySpace}
            />
          }
          {
          ...
          }
          {
            ...
            <OperationCard
              img={EditPng}
              tipText={t(Strings.share_login_tip)}
              btnText={t(Strings.login)}
              onClick={handleLogin}
            />
          }
