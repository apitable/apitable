import {
  BasicValueType, Field, FieldType, getTextFieldType, ICellValue, IField,
...
const SINGLE_TEXT_TYPE = [FieldType.Formula, FieldType.Number, FieldType.Currency, FieldType.Percent, 
  FieldType.DateTime];
        ...
        visibleFields.map((item, index) => {
          ...
          if (index === 0) {
            return (
              <div
                key={recordId}
                className={classNames(styles.cellTitle, styles.cellValue, 'ellipsis')}
              >
          ...
          // 相册类包括相册，架构视图
          const fieldValueStyle: React.CSSProperties = isVirtual ? {} : isGallery ?
