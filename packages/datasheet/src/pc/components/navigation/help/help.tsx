...
import {
  CourseOutlined, VikabyOutlined,
  RoadmapOutlined, ClassroomOutlined, JoinOutlined,
  InformationLargeOutlined, AdviseOutlined, BookOutlined,
  InformationSmallOutlined, WebsiteOutlined, DownloadOutlined, CommunityOutlined,
  KeyboardShortcutsOutlined, GuideOutlined, EditDescribeOutlined, SolutionOutlined, DemoOutlined, CodeFilled, ViewContactOutlined
} from '@vikadata/icons';
...
export const Help: FC<IHelpProps> = ({ className, templateActived }) => {
  ...
  const menuData = [
    [{
      ...
      onClick: () => navigationToUrl(
        isPrivateDeployment()
          ? `${window.location.origin}/help/developers`
          : Settings.developers_center_url.value
      ),
    }, {
  ...
  const getMobileMenuData = () => {
    return [menuData[0].reduce((prev, value, index) => {
  ...
  const HelpMenu = () => {
    return (
      <div className={styles.helpMenu}>
        <Typography className={styles.title} variant="h8" color={colors.fc1}>{t(Strings.help)}</Typography>
