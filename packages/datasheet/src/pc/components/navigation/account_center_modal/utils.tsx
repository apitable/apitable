...
type IGetVerifyDataFuncRes =
  | IChangePasswordConfig
  | IRefreshConfigConfig
  | IChangeMainAdminConfig
  | IDelSpaceConfig
  | null;

export const getVerifyData: ({
  key
}: {
  key: VerifyTypes;
}) => IGetVerifyDataFuncRes = ({ key }) => {
  const state = store.getState();
  const user = state.user.info;
  const mainAdminInfo = state.spacePermissionManage.mainAdminInfo;
  const sourceInfo =
    key === VerifyTypes.CHANGE_MAIN_ADMIN ? mainAdminInfo : user;
  const hasMobile = sourceInfo?.mobile;
  const codeMode = hasMobile
    ? ConfigConstant.LoginMode.PHONE
  ...
  switch (key) {
    case VerifyTypes.CHANGE_PASSWORD: {
      ...
      // 短信类型
      let smsType: ConfigConstant.SmsTypes | undefined =
        ConfigConstant.SmsTypes.MODIFY_PASSWORD;
      if (user?.mobile) {
    ...
    case VerifyTypes.REFRESH_TOKEN: {
      ...
      // 短信类型
      let smsType: ConfigConstant.SmsTypes | undefined =
        ConfigConstant.SmsTypes.CHANGE_DEVELOPER_CONFIG;
      if (user?.mobile) {
    ...
    case VerifyTypes.CHANGE_MAIN_ADMIN: {
      ...
      // 短信类型
      let smsType: ConfigConstant.SmsTypes | undefined =
        ConfigConstant.SmsTypes.CHANGE_MAIN_ADMIN;

      if (mainAdminInfo?.mobile) {
        inputText = `${mainAdminInfo!.areaCode}-${hiddenMobile(
          mainAdminInfo!.mobile
        )}`;
    ...
    case VerifyTypes.DEL_SPACE: {
      ...
      // 短信类型
      let smsType: ConfigConstant.SmsTypes | undefined =
        ConfigConstant.SmsTypes.DEL_SPACE;
      if (user?.mobile) {
        title = t(Strings.send_verification_code_to, {
          mobile: `${user?.areaCode} ${hiddenMobile(user!.mobile)}`
        });
