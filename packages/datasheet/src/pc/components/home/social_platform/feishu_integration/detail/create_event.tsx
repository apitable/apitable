...
export const CreateEvent: React.FC<ICreateEvent> = (props) => {
  ...
  const handleChange = useCallback((e, item: IFormItem) => {
  ...
  const setLarkConfig = useCallback(async(formData: IConfigForm) => {
    setLoading(true);
  ...
  return (
    <div className={classNames(
      ...
      <div className={styles.formWrap}>
        <div className={styles.form}>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: t(Strings.lark_integration_step4_content) }}
          />
      ...
      <div className={styles.buttonWrap}>
        <Button color="primary" onClick={onClick} disabled={!isValidForm} block loading={loading}>{t(Strings.lark_integration_step4_next)}</Button>
      </div>
    </div>
  );
};
