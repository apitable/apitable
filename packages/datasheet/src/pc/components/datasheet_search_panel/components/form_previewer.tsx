...
export const FormPreviewer: React.FC<IFormPreviewerProps> = (props) => {
  ...
  const onFormCreate = () => {
    const viewName = currentView.name ?
      `${currentView.name}${t(Strings.key_of_adjective)}${t(Strings.vika_form)}` :
      undefined;
  ...
  return (
    <div className={classnames(styles.formPreviewer, {
      ...
      <div className={styles.scrollContainer}>
        {
          canCreate ?
            <>
              <h3 className={styles.panelTitleDesc}>
                {t(Strings.preview_form_title_desc)}：
              </h3>
              <h3 className={styles.panelTitle}>
                {t(Strings.preview_form_title)}
              </h3>
