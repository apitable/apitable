...
import { INodeInstalledWidget } from '..';
...
export const WidgetPreview: React.FC<IWidgetPreviewProps> = (props) => {
  ...
  return <div className={styles.widgetList}>
    <h2>
      {t(Strings.datasheet_exist_widget)}
    </h2>
    <div className={styles.widgetListWrap}>
      ...
      >
        {
          installedWidgets.length ? installedWidgets.map(item => {
            return <div
              className={classNames({
                [styles.selected]: selectedWidgetIds.includes(item.widgetId),
              }, styles.widgetItem)}
              onClick={() => { selected(item.widgetId); }}
            >
              ...
              <div style={{ borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
                ...
                <Image src={item.widgetPackageCover || integrateCdnHost(Settings.widget_default_cover.value)} alt="" width={'100%'} height={100} />
              </div>
    ...
    <div style={{ position: 'relative', flexShrink: 0, zIndex: 1 }}>
      <Button color="primary" block disabled={!selectedWidgetIds.length} onClick={() => { onChange({ widgetIds: selectedWidgetIds }); }}>
