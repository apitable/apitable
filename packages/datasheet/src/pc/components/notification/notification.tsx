...
export const Notification: FC = () => {
  ...
  const { run: getAllData, loading: firstLoading } = useRequest(() => Api.getNotificationStatistics().then(async res => {
  ...
  const noticeListRended = (e, tabKey: string) => {
    const lastNotice = tabKey === TabKey.Unprocessed ? unReadNoticeList[unReadNoticeList.length - 1] :
      readNoticeList[readNoticeList.length - 1];
  ...
  const renderListBottom = (tabKey: ITabKeyType) => {
    ...
    const numEqual = tabKey === TabKey.Unprocessed ? unReadCount === unReadNoticeList.length :
      readCount === readNoticeList.length;
    ...
    const dom = numEqual ? t(Strings.end) : moreLoading ? (
    ...
    return (
      <div
        className={styles.tabBottomText}
        style={{ margin: numEqual ? '24px 0 48px' : '14px 0 38px', visibility: visible }}
      >
        {dom}
      </div>
    );
    ...
    if (tabActiveKey !== TabKey.Processed && unReadCount > 0) {
      return (
        <div className={styles.allToReadWrap}>
          {isMobile ? (allToReadBtnLoading ? (
            <Loading />
          ) : (
            <TextButton
              onClick={allToRead}
              prefixIcon={<ToReadIcon fill="currentColor" />}
            >
  ...
  return (
    <div className={DOM_WRAP_CLS}>
          ...
          <TabPane tab={t(Strings.unprocessed) + `(${unReadCount})`} key={TabKey.Unprocessed}>
            {
              ...
              <div
                ...
                id={tabActiveKey === TabKey.Unprocessed ? NOTIFICATION_ID.NOTICE_LIST_WRAPPER : ''}
              >
                {
                  unReadNoticeList.length > 0 &&
                  <div className={styles.cardWrapper}>
                    <QueueAnim ease="easeInQuint" duration={500} onEnd={e => noticeListRended(e, TabKey.Unprocessed)}>
                      {unReadNoticeList.map(item => <Card key={item.id} data={item} />)}
                    </QueueAnim>
                  </div>
                }
              ...
              >
                {
                  ...
                  <div className={styles.cardWrapper}>
                    <QueueAnim ease="easeInQuint" duration={500} onEnd={e => noticeListRended(e, TabKey.Processed)}>
                      {readNoticeList.map(item => <Card key={item.id} data={item} isProcessed />)}
                    </QueueAnim>
                  </div>
                }
      ...
      <div
        className={styles.newMsgFromWs}
        onClick={toNewMsg}
        style={{ visibility: newNoticeListFromWs.length > 0 ? 'visible' : 'hidden' }}
      >
