...
export const UnitPermissionSelect: React.FC<IUnitPermissionSelectProps> = (props) => {
  ...
  const unitMap = useSelector(state => {
  ...
  return <div className={classnames(styles.unitPermissionSelectWrapper, classNames)}>
      ...
      <div className={styles.unitValue} onClick={openMemberList} tabIndex={-1}>
        ...
        </div>
        {
          unitValue.map((unitId, index) => {
            ...
            return <MemberItem unitInfo={unitInfo} key={unitId}>
              <div
                onClick={e => {
                  stopPropagation(e);
                  removeUnit(unitId);
