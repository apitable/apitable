...
export const Welcome: FC = () => {
  ...
  const { treeNodesMap, rootId } = useSelector((state: IReduxState) => ({
  ...
  const data = [{
    ...
    rightBtn: isWecomSpace ? null : <TextButton className={styles.moreTemplateBtn}
      onClick={() => navigationToUrl(Settings.education_url.value, { method: Method.NewTab })}>
      <Typography variant="body4" color={colors.fc3}>{t(Strings.more_education)}</Typography>
    ...
    rightBtn: <TextButton className={styles.moreTemplateBtn} onClick={() => navigationTo({ path: Navigation.TEMPLATE })}>
      <Typography variant="body4" color={colors.fc3}>{t(Strings.more_template)}</Typography>
      <ChevronRightOutlined size={16} color={colors.fc3} />
    </TextButton>,
    modules: [{
      id: 'welcome_module_4',
    ...
    rightBtn: <TextButton className={styles.moreTemplateBtn}
      onClick={() => navigationToUrl(`${window.location.origin}${Settings.wecom_bind_help_center.value}${plm}`)}>
      <Typography variant="body4" color={colors.fc3}>{t(Strings.welcome_more_help)}</Typography>
      <ChevronRightOutlined size={16} color={colors.fc3} />
    </TextButton>,
    modules: [{
      id: 'welcome_module_8',
          ...
          <>
            <ComponentDisplay minWidthCompatible={ScreenSize.md}>
              <div className={styles.guide}>
                <Typography variant="h1" color={colors.fc1}>{t(Strings.welcome_title)}</Typography>
                        ...
                        <div className={styles.guideItem} key={index}>
                          {!isMobile &&
                            <div className={styles.titleWrapper}>
                              <Typography className={styles.title} variant="body1" color={colors.fc2}>{item.title}</Typography>
                              {item.rightBtn &&
                          ...
                          <div className={styles.moduleContainer}>
                            {(item.modules as any).map(module => (
                              ...
                              >
                                {module.isVideo && <div className={styles.videoBtn}><PlayFilled size={20} /></div>}
                                <span className={styles.moduleImg}>
                                  <Image src={module.img} alt={module.name} />
                                </span>
                                <Typography variant="body3" className={styles.moduleName} color={module.color}>{module.name}</Typography>
                              </div>
                            ))}
                          </div>
                        </div>
            ...
            <ComponentDisplay maxWidthCompatible={ScreenSize.md}>
              <div className={styles.mobileGuide}>
                <div>
                  <Typography variant="h2" color={colors.fc1}>{t(Strings.welcome_title)}</Typography>
                  <div className={styles.container}>
                    {data.map(item => (item.modules as any).map(module => (
                      <div
                        key={module.id}
                        className={styles.moduleItem}
                        style={{ backgroundColor: downModuleId === module.id ? module.activeBackgroundColor : module.backgroundColor }}
