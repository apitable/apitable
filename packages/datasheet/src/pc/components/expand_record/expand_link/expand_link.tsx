...
const ExpandLinkBase: React.ForwardRefRenderFunction<IExpandFieldEditRef, IExpandLinkProps> = (props, ref) => {
  const {
    field, recordId, onClick, editable, keyPrefix,
    addBtnText, rightLayout = true, onSave, datasheetId, manualFetchForeignDatasheet, mirrorId
  } = props;
  ...
  const hasShowMoreBtn = Boolean(
    cellValue &&
    cellValue.length &&
    cellValue.length > DEFAULT_LINK_RECORD_COUNT &&
    !isShowMoreOpen
  );
  ...
  useImperativeHandle(ref, (): IExpandFieldEditRef => {
    ...
    return {
      focus: (preventScroll?: boolean) => { editor.focus({ preventScroll }); },
      setValue: noop,
      saveValue: noop,
    };
  });
  ...
  function renderButton() {
    if (
      (!editable || field.property.limitSingleRecord) &&
    ...
    return (
      <Button
        className={style.addLinkRecordBtn}
        size="small"
        onClick={onAddRecord}
        disabled={!editable}
      >
        <span className={style.inner}>
          ...
          {
            !editable ? t(Strings.add_link_record_button_disable) : (
              addBtnText ||
              <TComponent
  ...
  return (
    ...
    >
      ...
      <div className={style.addLinkRecord}>
        {
          renderButton()
        }
      </div>
        ...
        foreignSnapshot && (
          <>
            <div className={style.recordList} onMouseDown={(e) => stopPropagation(e)}>
              {
                showCellValues && showCellValues.map((recordId, index) => {
              ...
              }
              {
                ...
                <div
                  ...
                  onClick={() => setIsShowMoreOpen(true)}
                >
                  <IconPullDown fill={colors.thirdLevelText} />
                  <span className={style.showMoreText}>{t(Strings.expand_rest_records_by_count, {
      ...
      {
        (datasheetLoading || foreignSnapshot) && <LinkEditor
          {...props}
          ref={editorRef}
          recordId={recordId}
          layout={
            (rightLayout && document.querySelector('.centerExpandRecord')) ? LinkEditorModalLayout.CenterRight : LinkEditorModalLayout.Center
          }
          style={visibleStyle}
          editing={editing}
