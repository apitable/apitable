...
//获取无权限人员列表
export const getNoPermissionMemberList = async(
  nodeId: string,
  unitsIds: string[],
) : Promise<IApi.INoPermissionMemberResponseData[] | null> => {
  if(!nodeId) {
    return null;
  }
    ...
    Message.warning({
      content: (
        <div>
          ...
          { manageable &&
            <i onClick={showSetPermission} >{t(Strings.notified_assign_permissions_text)}</i>
          }
        </div>
      ),
      onClose: closeModal,
  ...
  return(
    ...
    >
      <div className={styles.permissionNotifcationModal}>
        ...
        <p><MessageContent members={members}/></p>
      </div>
      <div className={styles.modalFooter}>
        { manageable ? 
          <>
            <p className={styles.cancel} onClick={closeModal}>{t(Strings.cancel)}</p>
            <div className={styles.line} />
            <p className={styles.confirm} onClick={showSetPermission}>{t(Strings.set_permission)}</p>
          </> : <p className={styles.know} onClick={closeModal}>{t(Strings.i_knew_it)}</p>
        }
...
const MessageContent = ({ members }) => {
  ...
  return (
    <>
      ...
      { members.length > 3 && t(Strings.notified_assign_permissions_number, { number: members.length }) }{t(Strings.unaccess_notified_message)}
    </>
  );
};
